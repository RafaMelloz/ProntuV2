...
export function SettingsPanel({ session, stripeStatus }) {
    ...
    const getOptions = () => {
        switch (controller) {
            ...
            case 3:
                return <ClinicConfig />;
            case 4:
                return <Payment stripeStatus={stripeStatus} />;
            default:
                return <Account />;
        }
    }
            ...
            <div className="flex gap-4 mb-8">
                {session.user.role === 'admin' ? (
                    <>
                        ...
                        <h3 onClick={() => handleController(4)} className={`text-lg md:text-xl font-semibold px-2 cursor-pointer ${controller === 4 ? 'text-azul-900 border-b-2 border-azul-900' : 'text-cinza-900 border-b-2 border-cinza-900'}`}>Plano</h3>
                    </>
                ) : <h3 onClick={() => handleController(1)} className={`text-lg md:text-xl font-semibold px-2 cursor-pointer ${controller === 1 ? 'text-azul-900 border-b-2 border-azul-900' : 'text-cinza-900 border-b-2 border-cinza-900'}`}>Conta e segurança</h3>}
            </div>
